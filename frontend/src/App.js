...
class App extends Component {
  ...
  tieredButtonFunction = (innerIndex, filterName, selectedValues) => {
    let lastSelected = this.state.lastSelected
    let lastClickUpdatedSelected = false
    if (innerIndex >= lastSelected) {
        lastSelected = innerIndex + 1
  ...
  render() {
    let buttons = [
      <TieredButtonController text="Ready to live?" filterName="HOUSE_TYPE" id={0} key={0} fatherStateFunction={this.tieredButtonFunction} fatherId={this.state.lastSelected} updateStateJSON={this.updateStateJSON} />,
      ...
      <TieredButtonController text="Ready to live?" filterName="ENUM_2" id={4} key={4} fatherStateFunction={this.tieredButtonFunction} fatherId={this.state.lastSelected} updateStateJSON={this.updateStateJSON} />
    ]
